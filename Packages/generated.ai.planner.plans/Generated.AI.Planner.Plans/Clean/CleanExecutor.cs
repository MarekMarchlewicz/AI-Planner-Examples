...
namespace Generated.AI.Planner.Plans.Clean
{
    ...
    public struct TerminationEvaluator : ITerminationEvaluator<StateData>
    {
        public bool IsTerminal(StateData state, out float terminalReward)
        {
            ...
            var OutOfBatteryInstance = new OutOfBattery();
            if (OutOfBatteryInstance.IsTerminal(state))
            {
                terminal = true;
                terminalReward += OutOfBatteryInstance.TerminalReward(state);
            }
            var CleanedAllInstance = new CleanedAll();
            if (CleanedAllInstance.IsTerminal(state))
            {
                terminal = true;
                terminalReward += CleanedAllInstance.TerminalReward(state);
            }
            return terminal;
        }
    }

    class CleanExecutor : BaseTraitBasedPlanExecutor<TraitBasedObject, StateEntityKey, StateData, StateDataContext, ActionScheduler, DefaultHeuristic, TerminationEvaluator, StateManager, ActionKey, DestroyStatesJobScheduler>
    {
        static Dictionary<Guid, string> s_ActionGuidToNameLookup = new Dictionary<Guid,string>()
        {
            { ActionScheduler.ChargeGuid, nameof(Charge) },
        ...
        protected override void Act(ActionKey actionKey)
        {
            ...
            {
                case var actionGuid when actionGuid == ActionScheduler.ChargeGuid:
                    actionName = nameof(Charge);
                    break;
            ...
            foreach (var argument in argumentMapping)
            {
                ...
                {
                    case nameof(Charge):
                        parameterIndex = Charge.GetIndexForParameterName(traitBasedObjectName);
                        break;
                    case nameof(Collect):
                ...
                if (split.Length > 1) // argument is a trait
                {
                    switch (split[1])
                    {
                        ...
                        case nameof(Location):
                            ...
                            break;
                        case nameof(ChargingStation):
                            var traitChargingStation = stateData.GetTraitOnObjectAtIndex<ChargingStation>(traitBasedObjectIndex);
                            arguments[i] = split.Length == 3 ? traitChargingStation.GetField(split[2]) : traitChargingStation;
                            break;
                        case nameof(Cleanliness):
                            var traitCleanliness = stateData.GetTraitOnObjectAtIndex<Cleanliness>(traitBasedObjectIndex);
                            arguments[i] = split.Length == 3 ? traitCleanliness.GetField(split[2]) : traitCleanliness;
                            break;
                        case nameof(Dirt):
        ...
        public override IActionParameterInfo[] GetActionParametersInfo(IStateKey stateKey, IActionKey actionKey)
        {
            ...
            {
                 case var actionGuid when actionGuid == ActionScheduler.ChargeGuid:
                    parameterNames = Charge.parameterNames;
                        break;
                 case var actionGuid when actionGuid == ActionScheduler.CollectGuid:
